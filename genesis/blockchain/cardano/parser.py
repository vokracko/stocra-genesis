...
class CardanoParser(Parser):
    ...
    async def decode_transaction(self, raw_transaction: dict) -> PlainTransaction:
        outputs = await self.get_outputs_with_amounts_from_raw_transaction(raw_transaction["inputs_outputs"])
        inputs = await self.get_inputs_from_raw_transaction(raw_transaction["inputs_outputs"])
        amount = await self.get_total_amount_from_raw_transaction(outputs)
    ...
    async def get_inputs_from_raw_transaction(self, inputs_outputs: List[dict]) -> List[PlainInput]:
        inputs = []
    ...
    async def get_outputs_with_amounts_from_raw_transaction(self, inputs_outputs: List[dict]) -> List[PlainOutput]:
        outputs = []
    ...
    async def get_total_amount_from_raw_transaction(self, outputs: List[PlainOutput]) -> Decimal:
